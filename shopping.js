...
function loadDataFromLocalStorage() {
    ...
    if (categoriesString == null) {
        ...
        categories = JSON.parse(computerCategoriesString);
    ...
    if (basketComputersString == null) {
        ...
        basketComputers = JSON.parse(basketCompuetrsString);
    }
...
function loadComputerCategories() {
    ...
    for (var i = 0; i <categories.length; i++) {
        ...
        computersCategoriesElementHTML += "<li class='list-group-item " +
    ...
    if (currentSelectedCategoryId === categoryId) { } else {
        ...
        for (let index = 0; index < categories.length; index++) {
            ...
            if (c.id === categoryId) {
                ...
                document.getElementById('computer-category-' + c.id).style.forWeight = 'normal';
            }
        }
        setTimeout(function () {
            ...
            if (computersSelected.length === 0) {
                computersElement.innerHTML = "<h2 class='not-found'>In this category the are no computers!</h2>"; 
            }
...
function refreshComputersBasket() {
    ...
    for (let index = 0; index < basketComputers.length; index++) {
        ...
        basketComputersTableBodyElementHtml += 'AZN</td><td><input min"1" max="10000" type="number" value="' +
            ...
            ComputersTableBodyElementHtml += '</td><td id="computer-total-price-' +
            b.computer.id + '">' + (b.computer.price * b.count);
            ComputersTableBodyElementHtml += 'AZN</td><td><button onclick="deleteBasketComputer(' +
            b.computer.id +
...
function onOrderSubmit(event) {
    ...
    for (let index = 0; index < orders.length; index++) {
        ...
        if (order.id > orderId) { orderId = orders.id; }
    }
    var userIdList = [];
...
function endOrderRegistration(customer) {
    ...
    closeConfirmOrder();
    setTimeout(() => {
        showAlertMessage("Your order is accepted!", 2000);
    }, 1000);
...
function addComputersToPage(computersLocal) {
    ...
    for (var i = 0; i < computersLocal.length; i++) {
        ...
        computersElementHTML += "<div class='computer-card-conatiner' >" +
        "<div class='computer-card' >" +
        ...
        "<div class='computer-description' title='" +
                ...
                "<div class='user-phone' title='" + c.phone + "'>" + c.phone + "</div>" +
                "<div class='add-to-basket-div'><button class='btn btn-primary' " +
                "onclick='onComputerSelected(" + c.id + ")'>More Details</button</div>"+
                "</div></div></div>";
    }
    computersElement.innerHTML = computersElementHTML;
}
...
function onSearchKeyDown(event) {
    if (event.keyCode == 13) {
        ...
        setTimeout(function () {
            ...
            computersElement.style.display = 'block';
            if (findedComputers.length == 0) { computersElement.innerHTML = "<h2 class='not found'>We couldn't find comuters that you searched for.</h2>!"}
            allComputersLoaded = false;
        }, 500);
...
window.addEventListener('scroll', () => {
    if (allComputersLoaded) {
        console.log('All computers are loaded!');
    } else {
        if (allowScroll) {
            ...
            if (distanceToBottom < clientHeight + 150) {
                ...
                setTimeout(function () {
                    ...
                    if (computersSelectedGlobal.length <= (begin + length)) {
                        ...
                        computersLoadingNextLoadingNext.style.display = 'none';
